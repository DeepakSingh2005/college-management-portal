...
const loginFacultyController = async (req, res) => {
  try {
    ...
    const user = await facultyDetails.findOne({ 
...
const updateMyFacultyDetailsController = async (req, res) => {
  try {
    ...
    if (password) {
      return ApiResponse.badRequest("Please use change-password endpoint to update password").send(res);
    }
...
module.exports = {
  ...
  updateMyFacultyDetailsController,
